 ...
 */
import { DirtyOptions, parseOptions, toTypeString, errString, _getStatusObj } from "./helper";
import { Listener, ListenerFunction } from "./listener";

type Evt = string | RegExp;

export class BetterEvents {
	public _betterEvents;


	constructor(options?: DirtyOptions) {
		this._betterEvents = {
			emitted: {}
			, after: {}
			, bufferEvt: {}
		...
		const errorHandler = this._betterEvents.options.onerror ?? logerror;
		Object.defineProperty(this._betterEvents, 'onerror', {
			configurable: true, value: (...args) => {
				try {
	...
	*/
	afterAll(events, callback, timeout, cancelOnTimeout) {
		if (!events as unknown instanceof Array || !events.length)
		...
		if (typeof timeout == 'number') {
			listener.timeout(() => {
				...
				const remaining = listener.remaining;
				if (!remaining.length) {
	...
	*/
	removeListener(listener: Listener | ListenerFunction, evt?: string | RegExp) {
		...
		const i = events.findIndex(obj => obj.listener == listener && String(obj.evt) == String(evt));
		if (i > -1)
	...
	* @private
	*/
	getListenersForEmit(evt, exclude) {

		const listeners = this.getListeners(evt, this._betterEvents.options.duplicate);
		...
		listeners.forEach(listener => {
			const group = (Object.hasOwnProperty.call(listener, 'i') ? listener['i'] : 0);
	...
	*                                                 promise as .promise
	*/
	emitEvent(evt, args?: any[] | any, options?: DirtyOptions, listenerEventFailed?: Error) {
		...
		const emitAs = _getEmitAs.call(this, options); //won't matter if callback is bound
